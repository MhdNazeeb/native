import {Image, ScrollView, StyleSheet, Text, View} from 'react-native';
import React from 'react';

export default function ElevetedCards() {
  return (
    <View>
      <Text style={styles.Text}>ElevetedCards</Text>
      <ScrollView horizontal={true} >
        <View style={styles.container}>
          <View style={[styles.card, styles.cardOne]}>
            <Image 
            style={{height:100,borderRadius:5}}
            source={{uri:'https://blog.castle.io/content/images/2021/03/blog-thumb-1.png'}}
            />
          </View>
          <View style={[styles.card, styles.cardTwo]}>
          <Image 
            style={{height:100,borderRadius:5}}
            source={{uri:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQP2F9MEccsSNABfd-9s0qYI4vGt-u8z4bxLggnC6EOGl6RDSQJr6ogqCZe0KipndZU0Ec&usqp=CAU'}}
            />
          </View>
          <View style={[styles.card, styles.cardThree]}>
          <Image 
            style={{height:100,borderRadius:5}}
            source={{uri:'https://res.cloudinary.com/practicaldev/image/fetch/s--KkP4AuFt--/c_imagga_scale,f_auto,fl_progressive,h_900,q_auto,w_1600/https://dev-to-uploads.s3.amazonaws.com/uploads/articles/ql92zpsre2li58mpjpsa.png'}}
            />
          </View>

          <View style={[styles.card, styles.cardFour]}>
          <Image 
            style={{height:100,borderRadius:5}}
            source={{uri:'https://bitnetinfotech.com/wp-content/uploads/2022/08/Frameworks-for-React-JS.jpg'}}
            />
          </View>
          <View style={[styles.card, styles.cardFive]}>
          <Image 
            style={{height:100,borderRadius:5}}
            source={{uri:'https://www.computerhope.com/jargon/j/javascript.png'}}
            />
          </View>
          <View style={[styles.card, styles.cardSix]}>
          <Image 
            style={{height:100,borderRadius:5}}
            source={{uri:'data:image/png;base64,'}}
            />
          </View>
        </View>
      </ScrollView>
    </View>
  );
}

const styles = StyleSheet.create({
  Text: {
    fontSize: 20,
    fontWeight: 'bold',
    margin:8
  },
  container: {flex: 1, marginLeft: 0, marginTop: 10, flexDirection: 'row'},
  card: {
    width: 100,
    height: 100,
    margin: 4,
    borderRadius: 5,
  },
  cardOne: {
    backgroundColor: '#FFE926',
  },
  cardTwo: {
    backgroundColor: '#0781FC',
  },
  cardThree: {
    backgroundColor: '#727D47',
  },
  cardFour: {
    backgroundColor: '#7C42AF',
  },
  cardFive: {
    backgroundColor: '#FFE926',
  },
  cardSix: {
    backgroundColor: '#FFE926',
  },
});
